# devops

1. [Devops solution](flowtools.md)
2. [Centos7 Install Redmine](redmine.md)
3. [Centos7 install git server](gut.md)
4. [uninstall php in CentOS](php.md)
5. [install and uninstall postgresql in CentOS](postgresql.md)
6. [install FTP CentOS](ftp.md)
7. [how to upload a file to github](upload.md)
8. [delete a github repository](deleterepository.md)
9. [install Oracle In CentOS](oracle.md)
10. [Process a MySQL cannot connect problem](connectfailed.md)
11. [restore MySQL a table ](restoretable.md)

